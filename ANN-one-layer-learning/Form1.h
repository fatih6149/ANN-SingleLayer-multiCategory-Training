...
#pragma endregion
	...
	private: System::Void continuousToolStripMenuItem_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		pictureBox1->Refresh();
		for (int i = 0; i < numSample; i++) {
			int temp_x = Convert::ToInt32(100 * Samples[i * inputDim] + (pictureBox1->Width >> 1));
			int temp_y = Convert::ToInt32((pictureBox1->Height >> 1) - 100 * Samples[i * inputDim + 1]);
		...
		do {
			cycle++;
			Error = delta_train(Samples, numSample, weights, bias, targets, numClass, inputDim);
			if (Error < Error_max)
				state = false;
		} while (state);
		label6->Text = " Cycle = " + System::Convert::ToString(cycle);
		...
		for (int label = 0; label < numClass; label++) {
			min_x = (this->pictureBox1->Width) / -2;
			min_y = YPoint(min_x, &weights[3 * label], 100);
			max_x = (this->pictureBox1->Width) / 2;
			max_y = YPoint(max_x, &weights[3 * label], 100);
